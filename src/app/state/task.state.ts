...
export class TasksState {
 ...
 static getError(state: TaskStateModel) {
  ...
  pinTask({ patchState, getState }: StateContext<TaskStateModel>, { payload }: PinTask) {
    ...
    const entities = {
      ...state,
      [payload]: { ...state[payload], state: state[payload].state ==  'TASK_PINNED' ? 'TASK_INBOX' : 'TASK_PINNED' },
    };
  ...
  archiveTask({ patchState, getState }: StateContext<TaskStateModel>, { payload }: ArchiveTask) {
    ...
    const entities = {
      ...state,
      [payload]: { ...state[payload], state: state[payload].state == 'TASK_ARCHIVED' ? 'TASK_INBOX' : 'TASK_ARCHIVED' },
    };
